...
| `duplicate_reviewed` | bool | Set by a **human** to answer a `duplicates-tracked-row` flag: this repo id shares a `names.repo_identity` with a row already in `models.yaml` (a dated snapshot of it, or its base against a tracked instruct tune), and a reviewer has confirmed the two really are different models. Must be a real YAML bool (`true`) — a quoted `"true"` or `no` is ignored on purpose, so a typo can never publish a row nobody cleared. Clears that reason **only**; every other gap still blocks promotion. To retire the older row instead, edit `models.yaml` by hand — `discover.py` only appends. Strip on promotion. |
